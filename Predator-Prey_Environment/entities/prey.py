...
class Prey(Animal):
    ...
    def __init__(
        ...
        self._thirst_max = 400
        self._thirst = self._thirst_max

        #self._reproduction_time = 400
    ...
    def get_observation(
        ...
        for predator in predators:
            self._process_entity(closest_predatos, predator, 2)

        predators_array = np.zeros(6)
        for i, (distance, angle, entity_type, pos_x, pos_y) in enumerate(closest_predatos):
            scaled_distance = scale_value(-distance, 0, math.sqrt(env_width**2 + env_height**2), 0, 1)
            scaled_angle = scale_value(angle, -180, 180, -1, 1)
            scaled_pos_x = scale_value(pos_x, 0, env_width, 0, 1)
            scaled_pos_y = scale_value(pos_y, 0, env_height, 0, 1)
            ...
            predators_array[i * 6:(i * 6) + 6] = [entity_type, scaled_distance, scaled_angle, scaled_difference, scaled_pos_x, scaled_pos_y]

        closest_entities = []
        for plant in plants:
            self._process_plant(closest_entities, plant, 1)
        ...
        result_array = np.append(result_array, predators_array)
        result_array = np.append(result_array, water_source_array)

        return result_array
    
    def _process_plant(self, closest_entities: list, plant: Plant, type: int):
        if not plant._is_grown:
            return
        
        self._process_entity(closest_entities, plant, type)
